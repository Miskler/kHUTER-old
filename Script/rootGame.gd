...
func _ready():
	...
	G.player_roster[get_tree().get_network_unique_id()] = G.game_settings["player_name"]
	if G.player_roster.has(0):
...
func spavn_players():
	...
	for i in G.player_roster.keys():
		...
		if G.live_player_roster.has(i) and str(i) != str(get_tree().get_network_unique_id()):
			...
			var GG = load("res://Sceni/NPS/Player_puppet.res").instance()
			GG.name = str(i)
...
remote func player_disconnect(id):
	if G.player_roster.has(id):
		...
		if get_node_or_null("Node/Player/Camera2D_specter") != null:
			get_node("Node/Player/Camera2D_specter/CanvasLayer/main").sistem_sms("Игрок " + str(G.player_roster[id]) + " отключился")
		else:
			get_node("Node/Player/Camera2D/interface/chat").sistem_sms("Игрок " + str(G.player_roster[id]) + " отключился")
...
remote func specter():
	var spect = load("res://Sceni/NPS/Specter.res").instance()
...
remote func revive_player(id):
	...
	if get_node_or_null("Node/Player/Camera2D_specter") != null and id != get_tree().get_network_unique_id():
		get_node("Node/Player/Camera2D_specter/CanvasLayer/main").sistem_sms("Игрок " + str(G.player_roster[id]) + " возрадился")
	elif get_node_or_null("Node/Player/Camera2D/interface/chat") != null and id != get_tree().get_network_unique_id():
		get_node("Node/Player/Camera2D/interface/chat").sistem_sms("Игрок " + str(G.player_roster[id]) + " возрадился")
	if id == get_tree().get_network_unique_id():
		if get_node_or_null("Node/Player") != null:
			get_node("Node/Player").queue_free()
		var player = load("res://Sceni/NPS/Player.res").instance()
		yield(get_tree(), "idle_frame")
		...
		var player_puppet = load("res://Sceni/NPS/Player_puppet.res").instance()
...
remote func player_died(id):
	...
	G.live_player_roster.erase(id)
	if get_node_or_null("Node/Player/Camera2D_specter") != null:
		get_node("Node/Player/Camera2D_specter/CanvasLayer/main").sistem_sms("Игрок " + str(G.player_roster[id]) + " умер")
	else:
		get_node("Node/Player/Camera2D/interface/chat").sistem_sms("Игрок " + str(G.player_roster[id]) + " умер")
	...
	for j in path.keys():
		if !j in no_use:
			if path[j] is String and(path[j].begins_with("res://")):
				...
				node_spavn.set(j, setter_param(param, path[j]))
			elif j != "use_setter_param":
				node_spavn.set(j, path[j])
	return node_spavn

remote func game_win(titre, text):
	var v = load("res://Sceni/win_game.res").instance()
	add_child(v)
	get_node("win/ColorRect2/Label").text = titre
	get_node("win/ColorRect2/RichTextLabel").text = text

remote func event_state(path, data):
